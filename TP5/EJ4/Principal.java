...
public class Principal {
    public static void main(String[] args){
        // Crear casas
        Gryffindor gryffindor = new Gryffindor(3, "Gryffindor");
        Slytherin slytherin = new Slytherin(3, "Slytherin");

        // Agregar cualidades
        gryffindor.agregarCualidad(new Cualidad("Valiente"));
        slytherin.agregarCualidad(new Cualidad("Ambicioso"));

        // Establecer enemistades
        gryffindor.agregarCasaEnemiga(slytherin);
        slytherin.agregarCasaEnemiga(gryffindor);

        // Crear alumnos
        Alumno harry = new Alumno("Harry");
        harry.agregarCualidad(new Cualidad("Valiente"));

        Alumno draco = new Alumno("Draco");
        draco.agregarCualidad(new Cualidad("Ambicioso"));

        // Crear Hogwarts y agregar casas
        Hogwarts hogwarts = new Hogwarts();
        hogwarts.agregarCasa(gryffindor);
        hogwarts.agregarCasa(slytherin);

        // Agregar alumnos
        hogwarts.agregarAlumno(harry);  // debería ir a Gryffindor
        hogwarts.agregarAlumno(draco);  // debería ir a Slytherin
    }
}
/*
4-  Modernizando el sombrero seleccionador
...
escuela. Para que una casa pueda aceptar a un nuevo alumno, además de tener lugar, el
...
requieren, sino que además en un afán de conservar los linajes de sangre pura, también exigen que el alumno cuente con un familiar que pertenezca en este momento a la misma
